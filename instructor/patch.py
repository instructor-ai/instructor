...
def handle_response_model(
    response_model: Optional[Type[BaseModel]], mode: Mode = Mode.TOOLS, **kwargs: Any
) -> Tuple[Union[Type[OpenAISchema], ParallelBase, None], Dict[str, Any]]:
    ...
    if response_model is not None:
        ...
        schema_dict = cast(Dict[str, Any], response_model.openai_schema)

        if mode == Mode.FUNCTIONS:
            new_kwargs["functions"] = [schema_dict]
            new_kwargs["function_call"] = {"name": schema_dict["name"]}
            ...
            new_kwargs["tool_choice"] = {
                "type": "function",
                "function": {"name": schema_dict["name"]},
            }
...
async def process_response_async(
    ...
    *,
    ...
    OpenAISchema, List[OpenAISchema], Dict[str, Any], Generator[Any, None, None]
]:
    """Processes a OpenAI response with the response model, if available.
