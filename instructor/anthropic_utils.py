...
def json_to_xml(json_str: str) -> str:
    ...
    references = model_dict["$defs"] if "$defs" in model_dict else None # Pass in nested structures if they exist
    _add_params(parameters, model_dict, references)

    return ET.tostring(root, encoding="unicode")

def _add_params(root: ET.Element, model_dict: Dict[str, Any], references: Dict[str, Any]) -> None: # check to handle nested params with the same name?
    ...
    for field_name, details in properties.items():
        ...
        if isinstance(details, dict) and "$ref" in details: # Checking if there are nested params
            ...
            _add_params(nested_params, _resolve_reference(references, details["$ref"]), references)
