...
def migrate_provider_calls(content: str) -> tuple[str, int]:
    ...
    def replace_openai_sync(match):
        ...
        if mode:
            return (
                f'instructor.from_provider("{DEFAULT_MODELS["openai"]}", mode={mode})'
            )
    ...
    def replace_openai_async(match):
        ...
        return (
            f'instructor.from_provider("{DEFAULT_MODELS["openai"]}", async_client=True)'
        )
    ...
    content = re.sub(
        pattern, f'instructor.from_provider("{DEFAULT_MODELS["google"]}")', content
    )

    # Pattern 6: from_groq
    pattern = r"instructor\.from_groq\(\s*(?:groq\.)?(?:Async)?Groq\([^)]*\)\s*\)"
    content = re.sub(
        pattern, f'instructor.from_provider("{DEFAULT_MODELS["groq"]}")', content
    )

    # Pattern 7: from_cohere
    pattern = (
        r"instructor\.from_cohere\(\s*(?:cohere\.)?(?:Async)?ClientV?2?\([^)]*\)\s*\)"
    )
    content = re.sub(
        pattern, f'instructor.from_provider("{DEFAULT_MODELS["cohere"]}")', content
    )

    # Pattern 8: from_cerebras
    pattern = r"instructor\.from_cerebras\(\s*(?:Async)?Cerebras\([^)]*\)\s*\)"
    content = re.sub(
        pattern, f'instructor.from_provider("{DEFAULT_MODELS["cerebras"]}")', content
    )

    # Pattern 9: from_fireworks
    pattern = r"instructor\.from_fireworks\(\s*(?:Async)?Fireworks\([^)]*\)\s*\)"
    content = re.sub(
        pattern, f'instructor.from_provider("{DEFAULT_MODELS["fireworks"]}")', content
    )
    ...
    content = re.sub(
        pattern,
        f'instructor.from_provider("{DEFAULT_MODELS["google"]}", vertexai=True)',
        content,
    )
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r"^\s*from\s+(openai|anthropic|groq|cohere)\s+import\s+", line):
            ...
            if provider == "openai" and not re.search(
                r"\bOpenAI\((?!.*from_provider)", rest_of_file
            ):
                skip_next_blank = True
                continue
            elif provider == "anthropic" and not re.search(
                r"\bAnthropic\((?!.*from_provider)", rest_of_file
            ):
                skip_next_blank = True
                continue
            elif provider == "groq" and not re.search(
                r"\bGroq\((?!.*from_provider)", rest_of_file
            ):
                skip_next_blank = True
                continue
            elif provider == "cohere" and not re.search(
                r"\bClient(?:V2)?\((?!.*from_provider)", rest_of_file
            ):
                skip_next_blank = True
                continue
    ...
    if content != original:
        changes = len(re.findall(r"from_provider", content)) - len(
            re.findall(r"from_provider", original)
        )
...
def process_file(filepath: Path, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        if not re.search(
            r"from_(openai|anthropic|gemini|groq|cohere|cerebras|fireworks|vertexai)\(",
            content,
        ):
            return False

        new_content, changes = migrate_provider_calls(content)
...
def main():
    ...
    parser.add_argument(
        "files", nargs="*", help="Specific files to process (default: all docs/*.md)"
    )
