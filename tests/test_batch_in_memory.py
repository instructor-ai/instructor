...
class TestBatchRequestInMemory:
    ...
    def test_invalid_buffer_type_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Unsupported file_path_or_buffer type"):
            batch_request.save_to_file(123, "openai")  # type: ignore[arg-type] # Invalid type  # ty:ignore[invalid-argument-type]
...
class TestProviderInMemorySupport:
    ...
    def test_provider_invalid_type_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Unsupported file_path_or_buffer type"):
            openai_provider.submit_batch(123)  # type: ignore[arg-type] # Invalid type  # ty:ignore[invalid-argument-type]

        with pytest.raises(ValueError, match="Unsupported file_path_or_buffer type"):
            anthropic_provider.submit_batch(123)  # type: ignore[arg-type] # Invalid type  # ty:ignore[invalid-argument-type]
