...
# Skip all tests if Mistral API key is not available
pytestmark = pytest.mark.requires_mistral
...
def test_image_size_validation(tmp_path: Path):
    """Test that images over 10MB are rejected."""
    large_image = tmp_path / "large_image.jpg"
...
@pytest.mark.parametrize("model, mode", product(models, modes))
def test_multiple_images(model, mode, client):
...
@pytest.mark.parametrize("model, mode", product(models, modes))
def test_image_downscaling(model, mode, client):
...
def test_base64_image_handling(base64_image):
    ...
    mistral_format = image.to_mistral()
    assert mistral_format["type"] == "image"
